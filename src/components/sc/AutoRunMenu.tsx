import { Check, CheckCircle2, ChevronDown, Hand } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            "inline-flex h-8 select-none items-center gap-1 rounded-full bg-surface-2 px-3 text-[12px] font-medium leading-none text-foreground/85 outline-none transition-colors",
      ...
      >
        ...
        >
          ...
          {autoMode === "auto" && <Check className="h-3.5 w-3.5" />}
        </DropdownMenuItem>
        <DropdownMenuItem
        ...
        >
          ...
          {autoMode === "confirm" && <Check className="h-3.5 w-3.5" />}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
